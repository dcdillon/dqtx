...
QUnixSignalHandler::QUnixSignalHandler() : m_notifier(nullptr)
{
    ...
    {
        throw std::runtime_error("Only one instance of QUnixSignalHandler per process.");
    }
    
    ::socketpair(AF_UNIX, SOCK_STREAM, 0, s_socketPair);
    m_notifier = new QSocketNotifier(s_socketPair[1], QSocketNotifier::Read, this);
    QObject::connect(m_notifier, SIGNAL(activated(int)), this, SLOT(onSignal(int)));
}
